...
# create a class so that multiple matrices may use the same calucaltion
class Matrix:
	# constructor
	...
	def check_zero_rows(self):
		...
		while count < self.nonzero_rows:			# go through every row that are now labeled as nonzero
			if np.all(self.matrix[count] == 0) == True:			# if the row is turned into a zero row by the previous operation
				...
				zero_count += 1				# number of rows removed
				self.nonzero_rows -= 1		# one fewer row labeled as nonzero
				self.total_row_swapped += 1			# now in total there's one row swapped
			else:
				count += 1
	...
	# the method returns the number of entries in each row starting from each pivot point
	...
	def check_length_rows(self):
		...
		# print(self.identity)
		i = 0
		while i < (self.nonzero_rows-1):		# the program goes through all rows to make sure that the descending order is consistent throughout the matrix
			if row_length[i+1] > row_length[i]:		# if the row higher up is longer than the row beneath
				row_length[i], row_length[i+1] = row_length[i+1], row_length[i]			# stor the swapped row lengths
	...
	# no matter what the user_choice is, th method runs through the rref calculation first
	# because we first need to know, through the rref value, whether the matrix is invertible
	# for inverse calculation, once we know that the matrix is invertible, we run through the method again, this time indicating that we're doing inverse calculations
	# when the method is run for the inverse calculation it runs the matrix, denoted A, along with the augmented identity matrix,
	...
	def calc(self, calc_type):			# calc_type indicates whether this is for rref calcualtions or inverse calcualtions
		...
		while i < self.nonzero_rows:			# for all nonzero rows
			...
			if self.len_check(i) == self.len_check(i-1):
				...
				self.matrix[i] = self.matrix[i] - (coef*self.matrix[i-1])			# subtract a multiple of the higher row from the lower row
			...
			for j in range(i+1, self.nonzero_rows):		# for all rows beneath the row(A) (that are shorter in length)
				for z in range(1,n):						# a constant that runs through all potential number of entries
					if self.len_check(i) == (self.len_check(j) + z):	# for all rows shoter than row(A)
						...
						self.matrix[i] = self.matrix[i] - (coef*self.matrix[j])			# subtract a multiple of the shorter row from the longer row
		...
		# return the result if the user chooses the rref calculation
		if calc_type == "rref":
			return self.matrix
		
		# if the user wchooses the inverse calculation
		elif calc_type == "inverse":
			# scenarios where the matrix is not invertible
			if m != n:				# if the matrix is not a square matrix
				...
				if one_count != m:			# if not all diagnal entries are 1 (if the rref of the matrix is not I)
				...
				else:				# if the rref is I, the matrix is invertible
					...
					# run through the function again, this time doing the inverse calculations with the indentity matrix augmented
